...
def broken_results(state, action):
    ...
    if standard_case.is_applicable(action):
        return [standard_case, state]
    else:
        return [standard_case]

CHANCE_OF_NO_ACTION = 0.9

def non_deterministic_noop_agent_type(level, initial_state, action_library, goal_description):
    # Create an action set for a single agent.
    action_library = action_library # remove the push actions
    ...
    while True:
        ...
        is_Noop = random.random() < CHANCE_OF_NO_ACTION
        is_applicable = current_state.is_applicable(joint_action)
        if is_Noop and is_applicable:
            joint_action = [action_set[0][0]]
            print(f"Ups! {joint_action_to_string(joint_action)}", flush=True, file=sys.stderr)
            print(joint_action_to_string(joint_action), flush=True)
            _ = parse_response(read_line())
            current_state = current_state.result(joint_action)
        else:
